...
```{r}
uglygardenharvest <- read_csv("https://raw.githubusercontent.com/llendway/scraping_etc/main/2020_harvest.csv")
```
...
```{r}
groceries_dataset2 <- read_csv("Groceries_dataset.csv")
```
...
```{r}
gen_req <- fall2017 %>% 
  html_elements("p:nth-child(2) , p:nth-child(2) strong") %>% 
  html_text2() %>% 
  str_sub(start = 35)
head(gen_req)
```
